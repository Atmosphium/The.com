...
exports.run = (Bot, Mesaj, Argüman) => {
  ...
  .addField ('**Üye Çıkış Mesajı nasıl ayarlanır?**', '`&bbmesaj Mesaj` yazarak ayarlayabilirsiniz.', true)
...
if (spamKoruması.has (Mesaj.author.id)) {
    ...
    if (!Mesaj.member.hasPermission ('MANAGE_MESSAGES')) {
      ...
      if (!Yazı) {
        ...
        Başarılı.setDescription ('Üye çıkış mesajı başarıyla ayarlandı!')
        Mesaj.channel.send (Başarılı)
        
        hafızaDosyası [Mesaj.guild.id] = {
          Çıkış: Yazı
        }
        hafızaDosyası [Mesaj.guild.id].Çıkış = Yazı
...
exports.Yapılandırma = {
  enabled: true,
  guildOnly: false,
  Lakap: ['BBMesajı', 'bb-mesajı', 'bbmesajı', 'bbmesaj'],
  yetkiSeviyesi: 0
}
